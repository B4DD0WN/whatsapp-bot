...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('B4DD0WN BOT', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    ...
    try {
            ...
            if (isMedia || isQuotedImage) {
              try {
	   	    ...
	   	    await aruga.reply(from, 'Maaf batas penggunaan hari ini sudah mencapai maksimal', id)
                ...
                if (!isUrl(url)) { await aruga.reply(from, 'link yang kamu kirim tidak valid.', id) }
                aruga.sendStickerfromUrl(from, url).then((r) => (!r && r !== undefined)
                    ? aruga.sendText(from, 'link yang kamu kirim tidak memuat gambar.')
        ...
        case 'stickergiphy':
	 {
            ...
            if (isGiphy) {
                ...
                await aruga.reply(from, 'command sticker giphy hanya bisa menggunakan link dari giphy.  [Giphy Only]', id)
            }
            break
        }
        case 'meme':
        ...
        case 'quotemaker':
            ...
            if (qmaker.length >= 3) {
                ...
                try {
                    ...
                    aruga.reply('Yahh proses gagal, isi yang bener! ..', id)
                }
            } else {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *B4DD0WN BOT* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *B4DD0WN BOT* 〙\n\n${msg}`)
            }
